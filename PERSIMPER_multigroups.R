PerSIMPER_m <- function(matrixSIMP, Groups, log = TRUE, leg = TRUE, count = TRUE, dataTYPE = "prab"){
  ...
  #If multiples groups are used to compute SIMPER and PER-SIMPER
  else
    {
    ContributionAll <- data.frame(x1 = AnaSimp[[1]]$average/sum(AnaSimp[[1]]$average))
    
    for(i in 2:length(AnaSimp))
      {
      ContributionAll <- cbind(ContributionAll, AnaSimp[[i]]$average/sum(AnaSimp[[i]]$average))
      }
    
    MoyenneContriPourcentage <- apply(ContributionAll, 1, mean)
    MoyenneContriPourcentage <- MoyenneContriPourcentage[]*100

    Pourcent_Contribution <- sort(MoyenneContriPourcentage, decreasing = TRUE)
    }
  ...
  for(i in 1:1000)
  {
    ...
    if(length(AnaSimp) == 1)
    {
      ...
      #Storage of SIMPER results (ranked contribution to OAD), if only 2 groups are analyzed
    ...
    {
      ContributionAll2 <- data.frame(x1 = simp2[[1]]$average/sum(simp2[[1]]$average))
      ContributionAll3 <- data.frame(x1 = simp3[[1]]$average/sum(simp3[[1]]$average))
      ContributionAll4 <- data.frame(x1 = simp4[[1]]$average/sum(simp4[[1]]$average))
      
      for(j in 2:length(simp2))
      {
        ContributionAll2 <- cbind(ContributionAll2, simp2[[j]]$average/sum(simp2[[j]]$average))
        ContributionAll3 <- cbind(ContributionAll3, simp3[[j]]$average/sum(simp3[[j]]$average))
        ContributionAll4 <- cbind(ContributionAll4, simp4[[j]]$average/sum(simp4[[j]]$average))
      }
      
      MoyenneContriPourcentage2 <- apply(ContributionAll2, 1, mean)
      MoyenneContriPourcentage2 <- MoyenneContriPourcentage2[]*100

      MoyenneContriPourcentage3 <- apply(ContributionAll3, 1, mean)
      MoyenneContriPourcentage3 <- MoyenneContriPourcentage3[]*100

      MoyenneContriPourcentage4 <- apply(ContributionAll4, 1, mean)
      MoyenneContriPourcentage4 <- MoyenneContriPourcentage4[]*100
      
      df2[i,] <- sort(MoyenneContriPourcentage2, decreasing = TRUE)
      df3[i,] <- sort(MoyenneContriPourcentage3, decreasing = TRUE)
      df4[i,] <- sort(MoyenneContriPourcentage4, decreasing = TRUE)
